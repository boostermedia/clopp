...
exports.preprocess = function (grunt, src, options)
{
    ...
    if (options.exclude)
    {
        contents = scanExcludes(contents, regex);
    }
    ...
    {
        contents = scanSearchDefinitions(contents, regex);
    }

    if (options.replaceDefinitions)
    {
        contents = scanReplaceDefinitions(contents, regex);
    }

    if (options.preprocess)
    {
        contents = scanIfDefBlocks(contents, regex);

        contents = scanIfNDefBlocks(contents, regex);

        contents = scanIfBlocks(contents, regex);
    }

    return contents;
};

function testCondition (condition, regexs) 
{
...
function scanSkips (source, regexs)
{
    var regexType = "array";
    var regexs_ = regexs;
 ...
 */
function scanSearchDefinitions (source, regexs)
{
    var regexType = "array";
    var regexs_ = regexs;
 ...
 */
function scanExcludes (source, regexs)
{
    var regexType = "array";
    var regexs_ = regexs;
 ...
 */
function scanReplaceDefinitions (source, regexs)
{
    var regexType = "array";
    var regexs_ = regexs;
 ...
 */
function scanIfDefBlocks (source, regexs)
{
    var regexType = "array";
    var regexs_ = regexs;
 ...
 */
function scanIfNDefBlocks (source, regexs)
{
    var regexType = "array";
    var regexs_ = regexs;
 ...
 */
function scanIfBlocks (source, regexs)
{
    var regexType = "array";
    var regexs_ = regexs;
    ...
    for (var i = 0; i < regexs.length; i++)
    {
        ...
        {
            var result = testCondition(test, regexs);
            ...
            if (result)
            {
                ...
                else
                {
                    ...
                    include = include.replace(/\n\/\* #endelseif \*\//g, function (match, replace) {
                        return "";
                    });
                    return include;
                }
            ...
            else
            {
                ...
                if (include.match(regex.elseif))
                {
                    ...
                    {
                        // Test each condition
                        var result = testCondition(conditions[i].condition);
