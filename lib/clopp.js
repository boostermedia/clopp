        ...
        if (regex.extraRegex)
        {
            contents = contents.replace(regex.extraRegex.if, function (match, test, include) 
            {
                ...
                if (result)
                {
                    ...
                    // Does the include contain #else or #elseif statements?
                    if (include.match(regex.extraRegex.elsechecker))
                    {
                        // Yes, the include contains #else & #elseif statements

                        // Remove all #else & #elseif statements from the include, and return that as a result
                        var include2;
                        include = include.replace(regex.extraRegex.elsechecker, function (match, actualInclude) {
                            include2 = actualInclude;
                        });
                        include = include2;
                    }
                ...
                else
                {
                    ...
                    // Does the include contain #elseif statements?
                    if (include.match(regex.extraRegex.elseif))
                    {
                        // Yes, the include does contain #elseif statements
                        ...
                        // Create an array of all conditions that are found
                        var conditions = [];
                        include = include.replace(regex.extraRegex.elseif, function (match, test, include) {
                            conditions.push({
                                condition: test,
                                include: include
                            });
                        ...
                        if (!found)
                        {
                            ...
                            // Does the include contain else statements?
                            if (include.match(regex.extraRegex.else))
                            {
                                // Yes, the include does contain #else statements

                                // Strip down the #else statement and return it
                                var include2;
                                include = include.replace(regex.extraRegex.else, function (match, actualInclude) {
                                    include2 = actualInclude;
                                });
                                include = include2;
                            }
